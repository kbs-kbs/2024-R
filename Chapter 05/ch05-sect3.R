...
hist(dist,                            # 자료(data)
     ...
     col="green",                     # 막대 색
     las=3,                           # x축 글씨 방향(0~3)
     breaks="Scott")                        # 막대 개수 조절

par(mfcol=c(2,2))
par(mfrow=c(1,1)) # 전체설정이라 초기화 해줌
...
# quantile의4분위와 박스플롯의 4분위는 계산 방법이 다르다
# 박스플롯에서는 이상치를 계산하기 때문에.
...
boxplot(Sepal.Length~Species, data=iris, main="품종별 꽃잎의 너비", las=1)
...
summary(mtcars)
par(mfrow=c(1,3))
# 단일변수 범주형(numeric인데 범주형에 가까움) 자료를 표현
